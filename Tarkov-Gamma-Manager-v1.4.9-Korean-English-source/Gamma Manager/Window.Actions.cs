    ...
    public partial class Window : Form
    {
        ...
        private async void buttonUpdateCheck_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (WebClient client = new WebClient())
                {
                    ...
                    Version currentVersion = new Version("1.4.9");
                    const int currentRevision = 10;
                    ...
                    {
                        MessageBox.Show(
                            LanguageManager.Korean ? "현재 최신 버전입니다.\r\n\r\n현재 버전: v1.4.9" : "You are using the latest version.\r\n\r\nCurrent version: v1.4.9",
                            LanguageManager.Korean ? "업데이트 확인" : "Check for Updates",
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
        ...
        private void buttonGameAuto_Click(object sender, EventArgs e)
        {
            ...
            using (GameAutoSettingsForm form = new GameAutoSettingsForm(iniFile, presets))
            {
                ...
                {
                    // IMPORTANT: saving Game Auto mappings is a configuration-only action.
                    // It must NEVER clear or rebuild live Hotkey/Toggle runtime state.
                    // A Hotkey/Toggle may already be active while the dialog is open,
                    // and its priority must remain intact after Save. Likewise, the
                    // Game Auto session snapshot belongs to the running game process,
                    // not to the settings dialog.
                    //
                    // Do not clear activeAutoPreset/manualOverride* here. Doing so makes
                    // an active Hotkey stop working and can also make the next foreground
                    // event treat the current 40/40 Toggle state as a fresh Game Auto/base
                    // state. Only the AutoGame_* mapping data was changed by the dialog.
        ...
        {
            string[] presets = iniFile.GetSections();
            // Global RegisterHotKey entries are explicitly unregistered while the
            // settings dialog is open, so pressing a configured hotkey cannot change
            // the active profile while the user is editing hotkey assignments.
            ...
            {
                // Always restore the current saved hotkeys, including after Cancel
                // or if the dialog is closed through the window close button.
                ResumeGlobalHotkeys();
            }
        }
        ...
        private void checkBoxLogEnabled_CheckedChanged(object sender, EventArgs e)
        {
            ...
            // Disable first so turning logging off does not itself append a log entry.
