    ...
    internal class IniFile
    {
        ...
        public void RenameSection(string oldSection, string newSection)
        {
            if (string.IsNullOrEmpty(oldSection) || string.IsNullOrEmpty(newSection) ||
                string.Equals(oldSection, newSection, StringComparison.Ordinal))
                return;

            string[] keys = { "monitor", "rGamma", "gGamma", "bGamma", "rContrast", "gContrast", "bContrast",
                "rBright", "gBright", "bBright", "saturation", "monitorBrightness", "monitorContrast", "Hotkeys" };

            foreach (string key in keys)
            {
                string value = Read(key, oldSection);
                if (!string.IsNullOrEmpty(value))
                    Write(key, value, newSection);
            }
