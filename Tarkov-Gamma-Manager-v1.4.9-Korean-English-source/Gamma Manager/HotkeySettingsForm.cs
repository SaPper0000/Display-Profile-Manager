    ...
    internal sealed class HotkeySettingsForm : Form
    {
        ...
        private void HotkeySettingsForm_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else
            {
                ...
                // A hotkey may belong to only one profile. If the requested key
                // is already assigned to another profile, offer to move it here.
                Row conflict = rows.Find(r => r != row && r.Key == e.KeyCode && r.Modifiers == modifiers);
                if (conflict != null)
                {
                    DialogResult result = MessageBox.Show(
                        LanguageManager.Korean
                            ? "이 핫키는 다른 프로필에서 사용 중입니다.\r\n\r\n[" + conflict.Preset + "]에 지정되어 있습니다.\r\n\r\n현재 프로필로 변경하시겠습니까?"
                            : "**This hotkey is already in use by another profile.**\r\n\r\nIt is assigned to [" + conflict.Preset + "].\r\n\r\nChange it to the current profile?",
