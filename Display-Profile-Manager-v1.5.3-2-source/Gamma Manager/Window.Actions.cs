            ...
            try
            {
                ...
                using (var request = new HttpRequestMessage(HttpMethod.Get, apiUrl))
                {
                    ...
                    using (var response = await _updateHttpClient.SendAsync(request))
                    {
                        ...
                        Version currentVersion = System.Reflection.Assembly.GetExecutingAssembly().GetName().Version ?? new Version(1, 5, 3, 2);
                        const int currentRevision = 2;
                        string currentVersionText = "v" + currentVersion.ToString(3) + "-2";
