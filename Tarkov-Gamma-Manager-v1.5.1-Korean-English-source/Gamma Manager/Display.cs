        ...
        public static List<DisplayInfo> QueryDisplayDevices()
        {
            ...
            (IntPtr hMonitor, IntPtr hdcMonitor, ref WinApi.RECT lprcMonitor, IntPtr dwData) =>
            {
                ...
                if (WinApi.GetMonitorInfo(hMonitor, ref monitorInfo))
                {
                    ...
                    int firstSlash = DName.IndexOf("\\");
                    if (firstSlash >= 0)
                    {
                        DName = DName.Substring(firstSlash + 1);
                        int secondSlash = DName.IndexOf("\\");
                        if (secondSlash >= 0)
                            DName = DName.Substring(0, secondSlash);
                    }
                    if (string.IsNullOrWhiteSpace(DName)) DName = cleanDeviceName;
