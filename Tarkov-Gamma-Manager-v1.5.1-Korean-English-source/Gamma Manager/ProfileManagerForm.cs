    ...
    internal sealed class ProfileManagerForm : Form
    {
        ...
        private void MoveProfilesToTop()
        {
            ...
            foreach (int index in indices)
            {
                object item = listProfiles.Items[index];
                listProfiles.Items.RemoveAt(index);
                listProfiles.Items.Insert(insertIndex, item);
                listProfiles.SetSelected(insertIndex, true);
                insertIndex++;
            }
            listProfiles.EndUpdate();
            changed = true; SaveProfileOrder();
        ...
        private void MoveProfilesToBottom()
        {
            ...
            var indices = listProfiles.SelectedIndices.Cast<int>().OrderByDescending(x => x).ToList();
            listProfiles.BeginUpdate();
            int insertIndex = listProfiles.Items.Count - 1;
            foreach (int index in indices)
            {
                object item = listProfiles.Items[index];
                listProfiles.Items.RemoveAt(index);
        ...
        private void ExportProfile()
        {
            ...
            string raw = string.Join("|", "TGM-Profile", "v1.5", EscapeField(p),
        ...
        private void ImportProfile()
        {
            ...
            try
            {
                ...
                iniFile.Write("rGamma", UnescapeField(parts[4]), pName);
                iniFile.Write("gGamma", UnescapeField(parts[5]), pName);
                iniFile.Write("bGamma", UnescapeField(parts[6]), pName);
                iniFile.Write("rContrast", UnescapeField(parts[7]), pName);
                iniFile.Write("gContrast", UnescapeField(parts[8]), pName);
                iniFile.Write("bContrast", UnescapeField(parts[9]), pName);
                iniFile.Write("rBright", UnescapeField(parts[10]), pName);
                iniFile.Write("gBright", UnescapeField(parts[11]), pName);
                iniFile.Write("bBright", UnescapeField(parts[12]), pName);
                iniFile.Write("saturation", UnescapeField(parts[13]), pName);
                iniFile.Write("monitorBrightness", UnescapeField(parts[14]), pName);
                iniFile.Write("monitorContrast", UnescapeField(parts[15]), pName);
