    ...
    internal class Gamma
    {
        ...
        public static ushort[,] CreateGammaRamp(float rGamma, float gGamma, float bGamma, float rContrast, float gContrast, float bContrast, float rBright, float gBright, float bBright)
        {
            ...
            for (int i = 0; i < 256; i++)
            {
                ...
                newGamma[0, i] = (ushort)Clamp((int)Math.Round(rVal * 256.0), 0, 65535); // r
                newGamma[1, i] = (ushort)Clamp((int)Math.Round(gVal * 256.0), 0, 65535); // g
                newGamma[2, i] = (ushort)Clamp((int)Math.Round(bVal * 256.0), 0, 65535); // b
            }
            return newGamma;
        }
