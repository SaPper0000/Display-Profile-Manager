...
namespace Gamma_Manager
{
    ...
    public partial class Window : Form
    {
        ...
        private bool isBrightnessCommunicating = false; // 추가: 통신 중복 방지
        private bool isContrastCommunicating = false; // 추가: 통신 중복 방지
        // ----------------------------------------
        ...
        private async void trackBarMonitorBrightness_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (!disableChangeFunc)
            {
                ...
                // --- 슬라이더를 마구 움직일 때 발생하는 렉 방지 (디바운싱 & 비동기) ---
                int currentToken = unchecked(++brightnessDebounceToken);
                await System.Threading.Tasks.Task.Delay(100); // 0.1초 대기 (드래그가 끝났는지 확인)
                if (currentToken != brightnessDebounceToken) return; // 아직 움직이는 중이면 무시

                // 이전 통신이 끝나지 않았다면 기다립니다 (스레드 병목 및 최종값 무시 방지)
                while (isBrightnessCommunicating)
                {
                    await System.Threading.Tasks.Task.Delay(50);
                    if (currentToken != brightnessDebounceToken) return; // 대기 중 새 값이 들어오면 취소
                }

                Display.DisplayInfo targetDisplay = currDisplay;
                int targetValue = trackBarMonitorBrightness.Value;

                isBrightnessCommunicating = true;
                await System.Threading.Tasks.Task.Run(() =>
                {
                    if (targetDisplay.isExternal)
                    {
                        ExternalMonitor.SetBrightness(targetDisplay.PhysicalHandle, (uint)targetValue);
                    }
                    else
                    {
                        InternalMonitor.SetBrightness((byte)targetValue);
                    }
                });
                isBrightnessCommunicating = false;
                // -------------------------------------------------------------------------
            }
        }
        ...
        private async void trackBarMonitorContrast_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (!disableChangeFunc)
            {
                ...
                // --- 슬라이더를 마구 움직일 때 발생하는 렉 방지 (디바운싱 & 비동기) ---
                int currentToken = unchecked(++contrastDebounceToken);
                await System.Threading.Tasks.Task.Delay(100);
                if (currentToken != contrastDebounceToken) return;

                // 이전 통신이 끝나지 않았다면 기다립니다
                while (isContrastCommunicating)
                {
                    await System.Threading.Tasks.Task.Delay(50);
                    if (currentToken != contrastDebounceToken) return; // 대기 중 새 값이 들어오면 취소
                }

                Display.DisplayInfo targetDisplay = currDisplay;
                int targetValue = trackBarMonitorContrast.Value;

                isContrastCommunicating = true;
                await System.Threading.Tasks.Task.Run(() =>
                {
                    ExternalMonitor.SetContrast(targetDisplay.PhysicalHandle, (uint)targetValue);
                });
                isContrastCommunicating = false;
                // -------------------------------------------------------------------------
            }
        }
