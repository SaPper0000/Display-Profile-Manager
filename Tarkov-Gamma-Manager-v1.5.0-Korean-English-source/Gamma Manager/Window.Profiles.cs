...
namespace Gamma_Manager
{
    ...
    public partial class Window : Form
    {
        ...
        private void ApplyPhysicalMonitorProfileValues(Display.DisplayInfo display, int brightness, int contrast, string profileName)
        {
            ...
            // Physical-monitor writes are always serialized by the caller. This is
            // important for rapid hotkeys: an older DDC/CI write may already be
            // in progress when a newer profile arrives. The newer request waits for
            // the older write to finish and then becomes the final hardware state.
        ...
        private void QueuePhysicalMonitorProfileValues(string monitorName, int brightness, int contrast, string profileName, int generation)
        {
            ...
            {
                lock (physicalMonitorApplyLock)
                {
                    // Skip requests that were never allowed to start because a newer
                    // hotkey/restore request superseded them while they were waiting.
                    if (generation != physicalMonitorApplyGeneration) return;

                    Display.DisplayInfo physicalDisplay = null;
                    ...
                    if (physicalDisplay == null) return;

                    // Once a hardware write starts it cannot be cancelled safely.
                    // Serialize all physical-monitor writes so the newest queued
                    // request always runs after any already-running older request.
                    ApplyPhysicalMonitorProfileValues(physicalDisplay, brightness, contrast, profileName);
                }
            });
        }
        ...
        private void ApplyPreset(string preset, bool switchMonitor, bool applyMonitorSettings)
        {
            ...
            Saturation.Apply(currDisplay, currDisplay.saturation);
                ...
                if (applyMonitorSettings)
                {
                    ...
                    // --- 변경된 부분: 모니터 제어를 백그라운드 스레드로 분리하여 렉 해결 ---
                    Display.DisplayInfo targetDisplay = currDisplay;
                    string targetPreset = preset;
                    System.Threading.Tasks.Task.Run(() =>
                    {
                        ApplyPhysicalMonitorProfileValues(targetDisplay, safeBrightness, safeContrast, targetPreset);
                    });
                    // --------------------------------------------------------------------
