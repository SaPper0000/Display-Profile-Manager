...
namespace Gamma_Manager
{
    ...
    public partial class Window : Form
    {
        ...
        private MonitorStateSnapshot CaptureMonitorState(Display.DisplayInfo display)
        {
            ...
            // 토글 직전의 실제 프로그램 상태를 기준으로 저장한다.
            // UI 슬라이더는 DDC/CI 비동기 읽기 때문에 순간적으로 50 등의
            // 오래된 값을 가지고 있을 수 있으므로 토글 원본값으로 사용하지 않는다.
            int currentBrightness = display.monitorBrightness;
            int currentContrast = display.monitorContrast;

            // 현재 모니터의 실제 하드웨어 값을 가능하면 다시 읽는다.
            if (display.isExternal &&
                display.PhysicalHandle != IntPtr.Zero &&
                display.PhysicalHandle != (IntPtr)(-1))
            {
                if (ExternalMonitor.TryGetBrightness(
                        display.PhysicalHandle,
                        out int liveBrightness))
                {
                    currentBrightness = liveBrightness;
                    display.monitorBrightness = liveBrightness;
                }

                if (ExternalMonitor.TryGetContrast(
                        display.PhysicalHandle,
                        out int liveContrast))
                {
                    currentContrast = liveContrast;
                    display.monitorContrast = liveContrast;
                }
            }
            else if (!display.isExternal)
            {
                if (InternalMonitor.TryGetBrightness(out int liveBrightness))
                {
                    currentBrightness = liveBrightness;
                    display.monitorBrightness = liveBrightness;
                }
            ...
            currentContrast = Math.Max(0, Math.Min(100, currentContrast));
