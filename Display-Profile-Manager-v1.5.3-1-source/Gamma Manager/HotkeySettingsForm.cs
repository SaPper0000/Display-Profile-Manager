    ...
    internal sealed class HotkeySettingsForm : Form
    {
        ...
        public HotkeySettingsForm(IniFile iniFile, string[] presets, List<Display.DisplayInfo> displays)
        {
            ...
            noticeBox.Paint += (s, e) =>
            {
                ...
                using (Font regularFont = new Font("Segoe UI", 9f, FontStyle.Regular))
                ...
                using (SolidBrush highlightBrush = new SolidBrush(Color.FromArgb(255, 195, 0))) // 👈 선명한 골드/노란색 강조
                {
                    ...
                    {
                        // 1줄
                        g.DrawString("※ 다른 프로그램(게임 등) 실행 중 핫키를 사용하려면", regularFont, normalBrush, new PointF(0, 2));
                        ...
                        SizeF szPrefix = g.MeasureString(prefix, regularFont);
                        SizeF szBold = g.MeasureString(boldText, boldFont);

                        g.DrawString(prefix, regularFont, normalBrush, new PointF(0, 22));
                        g.DrawString(boldText, boldFont, highlightBrush, new PointF(szPrefix.Width - 5, 22));
                        g.DrawString(suffix, regularFont, normalBrush, new PointF(szPrefix.Width + szBold.Width - 10, 22));
                    }
                    else
                    {
                        g.DrawString("※ To use hotkeys while other apps (or games) are focused,", regularFont, normalBrush, new PointF(0, 2));
            ...
            if (displays != null)
            {
                ...
                for (int i = 0; i < displays.Count; i++)
                {
                    ...
                    AddSpecialRow(HARD_RESET_SINGLE_PREFIX + monitorName, LanguageManager.Korean ? $"🔄 {i + 1}) {monitorName} 초기화" : $"🔄 Reset {i + 1}) {monitorName}", false);
                    AddSpecialRow(CYCLE_SINGLE_PREFIX + monitorName, LanguageManager.Korean ? $"🔁 {i + 1}) {monitorName} 프로필 순환" : $"🔁 Cycle {i + 1}) {monitorName}", true);
                }
            }
        ...
        private void AddSpecialRow(string presetName, string displayName, bool isCycle)
        {
            ...
            TryParseHotkey(iniFile.Read(presetName, "Hotkeys"), out Keys key, out GlobalHotkey.Modifiers modifiers);
            Row row = new Row { Preset = presetName, SetButton = setButton, ClearButton = clearButton, ModeCombo = null, CycleCheck = null, Key = key, Modifiers = modifiers };
