        ...
        private bool TryGetAutoMapping(string foregroundExe, Display.DisplayInfo fgDisplay, out string profile)
        {
            ...
            foreach (string section in sections)
            {
                    ...
                    string.Equals(Path.GetFileName(exe), Path.GetFileName(foregroundExe), StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (fgDisplay != null && (!string.IsNullOrEmpty(mappedHwId) || !string.IsNullOrEmpty(mappedMon)))
                    {
                        ...
                        bool matchName = !string.IsNullOrEmpty(mappedMon) && !string.IsNullOrEmpty(fgDisplayName) &&
                                         string.Equals(mappedMon, fgDisplayName, StringComparison.OrdinalIgnoreCase);

                        if (!matchHwId && !matchName)
                            continue;
                    }
