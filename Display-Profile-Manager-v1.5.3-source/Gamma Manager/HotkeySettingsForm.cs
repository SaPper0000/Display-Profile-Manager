...
namespace Gamma_Manager
{
    internal sealed class HotkeySettingsForm : Form
    {
        ...
        public HotkeySettingsForm(IniFile iniFile, string[] presets, List<Display.DisplayInfo> displays)
        {
            ...
            status = new Label { AutoSize = true, Location = new Point(12, 382) };

            Button save = new Button { Text = LanguageManager.Korean ? "저장" : "Save", Size = new Size(90, 30), Location = new Point(492, 452) };
            save.Click += delegate { SaveAndClose(); };

            Button cancel = new Button { Text = LanguageManager.Korean ? "취소" : "Cancel", Size = new Size(90, 30), Location = new Point(592, 452) };
            cancel.Click += delegate { DialogResult = DialogResult.Cancel; Close(); };

            Controls.AddRange(new Control[] { info, panel, status, save, cancel });
