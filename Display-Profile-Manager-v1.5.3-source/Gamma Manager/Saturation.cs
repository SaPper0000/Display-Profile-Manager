...
namespace Gamma_Manager
{
    ...
    internal static class Saturation
    {
        ...
        // 👈 전역 초기화/종료 및 바인딩 관리용 전역 락
        private static readonly object _globalInitLock = new object();

        // 👈 GPU Vendor/Adapter별 독립 동시 제어를 위한 세분화 락 (10순위 개선)
            ...
            lock (adapterLock)
            {
                try
                {
                    ...
                    if (display.adapterVendor == WinApi.DisplayAdapterVendor.Nvidia)
                    {
                        ...
                        {
                            lock (_globalInitLock) { bindings[display.displayLink] = b; }
                            ...
                            if (GetNvidia(display, b, out current, out def, out min, out max, out step))
                            {
                                ...
                                lock (_globalInitLock)
                                {
                                    if (!originalValues.ContainsKey(display.displayLink))
                                        originalValues[display.displayLink] = current;
                                }
                                return true;
                            }
                        }
        ...
        public static bool Apply(Display.DisplayInfo display, int value)
        {
            ...
            // 👈 타 모니터와 병목 없이 GPU/어댑터별 독립 락만 점유하여 병렬 적용
            object adapterLock = GetLockForDisplay(display);
            lock (adapterLock)
            {
                Binding b;
                lock (_globalInitLock)
                {
                    if (!bindings.TryGetValue(display.displayLink ?? string.Empty, out b)) return false;
                }
        ...
        /// <summary>
        /// 비정상 종료 복구 시 올바른 원본 기준값으로 오리지널 캐시를 동기화합니다.
        /// </summary>
        ...
        private static bool EnsureNvidia()
        {
            ...
            lock (_globalInitLock)
            {
                ...
                try
                {
                    ...
                    if (nvInitialize == null || nvGetAssociatedDisplayHandle == null || nvGetDvcInfo == null || nvSetDvcLevel == null) { CleanupNvidiaLoadFailure(); return false; }
                    if (nvInitialize() != NVAPI_OK) { CleanupNvidiaLoadFailure(); return false; }
                    nvReady = true;
                    return true;
                }
                catch
        ...
        private static bool EnsureAmd()
        {
            ...
            lock (_globalInitLock)
            {
                ...
                try
                {
                    ...
                    if (adlCreate == null || adlAdapterCount == null || adlDisplayInfoGet == null || adlColorGet == null || adlColorSet == null) { CleanupAmdLoadFailure(); return false; }
                    if (adlCreate(adlAlloc, 1) != 0) { CleanupAmdLoadFailure(); return false; }
                    adlReady = true;
                    return true;
                }
                catch
