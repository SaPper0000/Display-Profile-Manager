...
namespace Gamma_Manager
{
    ...
    internal class DisplayService
    {
        ...
        // 모니터별 식별 키 추출 (불변 hardwareId 우선)
        public static string GetMonitorKey(Display.DisplayInfo display)
        {
            if (display == null) return string.Empty;
            return !string.IsNullOrEmpty(display.hardwareId) ? display.hardwareId : display.displayName;
        }
            ...
            int targetGeneration)
        {
            ...
            // 1. 대상 모니터 먼저 안전하게 탐색 (hardwareId -> displayName 순)
        ...
        public void ResetPhysicalMonitorAsync(Display.DisplayInfo display, int targetGeneration)
        {
            ...
            string monitorKey = GetMonitorKey(display);
            int targetTopologyGen = CurrentTopologyGeneration;
                ...
                lock (_physicalApplyLock)
                {
                    ...
                    if (display.isExternal)
                    {
                        ...
                        if (display.PhysicalHandle != IntPtr.Zero && display.PhysicalHandle != (IntPtr)(-1))
                        {
                            ...
                            if (!ok && targetTopologyGen == CurrentTopologyGeneration && Display.RefreshPhysicalMonitorHandle(display))
                            {
                    ...
                    {
                        InternalMonitor.SetBrightness(display.hardwareId, (byte)defaultVal);
                    }
                }
            });
        }
