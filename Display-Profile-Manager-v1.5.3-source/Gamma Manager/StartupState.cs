        ...
        public static bool Capture(List<Display.DisplayInfo> displays)
        {
            try
            {
                ...
                foreach (Display.DisplayInfo display in displays)
                {
                    ...
                    saved.Saturation = display.saturation; // 👈 현재 원본 채도 저장
                    int brightness;
                    int contrast;
                    ...
                    {
                        saved.MonitorBrightness = InternalMonitor.TryGetBrightness(display.hardwareId, out brightness) ? brightness : display.monitorBrightness;
