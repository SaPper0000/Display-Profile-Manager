...
namespace Gamma_Manager
{
    internal class Display
    {
        ...
        public class DisplayInfo
        {
            ...
            public string hardwareId = string.Empty; // 👈 불변의 하드웨어 고유 식별자 (EDID/PNP 기반)
        ...
        {
            InternalMonitor.Cleanup();  // WMI 인스턴스 캐시 초기화
            ...
            (IntPtr hMonitor, IntPtr hdcMonitor, ref WinApi.RECT lprcMonitor, IntPtr dwData) =>
            {
                ...
                if (WinApi.GetMonitorInfo(hMonitor, ref monitorInfo))
                {
                    ...
                    {
                        // 0번부터 더 이상 장치가 없을 때까지 표준 방식으로 모니터 모델명 및 Hardware PNP ID 탐색
                        for (uint devNum = 0; ; devNum++)
                        {
                            ...
                            if (!WinApi.EnumDisplayDevices(cleanDeviceName, devNum, ref monitorDevice, 0))
                                break; // 더 이상 열거할 디바이스가 없으면 안전하게 종료
                    ...
                    // hardwareId가 비어있는 경우 displayName 또는 displayLink로 안전 fallback
                ...
                if (GetNumberOfPhysicalMonitorsFromHMONITOR(hMonitor, ref physicalMonitorsCount) && physicalMonitorsCount > 0)
                {
                ...
                {
                    Logger.Warn("GetNumberOfPhysicalMonitorsFromHMONITOR returned 0 or failed for " + monitor.displayName);
                }
                ...
                {
                    // 👈 hardwareId 매칭 기반 개별 WMI 밝기 조회
                    int brightness;
                    monitor.monitorBrightness = InternalMonitor.TryGetBrightness(monitor.hardwareId, out brightness) ? brightness : 50;
        ...
        public static bool RefreshPhysicalMonitorHandle(DisplayInfo monitor)
        {
            ...
            // 모니터 객체 자체를 lock하여 동일 모니터에 대한 핸들 갱신 및 조회/적용 간 교체 경쟁 원천 차단
            lock (monitor)
            {
                uint count = 0;
