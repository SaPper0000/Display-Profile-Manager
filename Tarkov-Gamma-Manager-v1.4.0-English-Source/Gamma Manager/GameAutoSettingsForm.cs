...
namespace Gamma_Manager
{
    internal sealed class GameAutoSettingsForm : Form
    {
        ...
        private void Build()
        {
            ...
            list.SelectedIndexChanged += delegate { UpdateMonitorCheckbox(); };
            Controls.Add(list);

            applyMonitor = new CheckBox { Text = ko ? "선택한 게임에서 모니터 밝기/대비도 자동 변경" : "Also change physical monitor brightness/contrast for the selected game", Location = new Point(20, 350), Size = new Size(500, 24) };
            applyMonitor.CheckedChanged += ApplyMonitorChanged;
            Controls.Add(applyMonitor);
            ...
            if (sections != null)
            {
                foreach (string section in sections)
                {
                    ...
                    if (!string.IsNullOrEmpty(profile) && !string.IsNullOrEmpty(exe))
                        mappings.Add(new Mapping { Profile = profile, Exe = Path.GetFileName(exe), Enabled = ini.Read("Enabled", section) != "0", ApplyMonitor = ini.Read("ApplyMonitor", section) == "1" });
                }
            }
            ...
            if (mappings.Count == 0 && presets != null)
            {
                foreach (string p in presets)
                {
                    ...
                    if (!string.IsNullOrEmpty(exe) && (en == "1" || string.Equals(en, "true", StringComparison.OrdinalIgnoreCase)))
                        mappings.Add(new Mapping { Profile = p, Exe = Path.GetFileName(exe), Enabled = true, ApplyMonitor = false });
                }
            }
            RefreshList();
        }
        ...
        {
            if (applyMonitor == null) return;
            int i = list.SelectedIndex;
            applyMonitor.CheckedChanged -= ApplyMonitorChanged;
            applyMonitor.Checked = i >= 0 && mappings[i].ApplyMonitor;
            applyMonitor.CheckedChanged += ApplyMonitorChanged;
        }

        private void ApplyMonitorChanged(object sender, EventArgs e)
        {
            if (list.SelectedIndex >= 0) mappings[list.SelectedIndex].ApplyMonitor = applyMonitor.Checked;
        }
        ...
        private void Add_Click(object sender, EventArgs e)
        {
            ...
            using (Form f = new Form())
            {
                ...
                CheckBox am = new CheckBox { Text = ko ? "모니터 밝기/대비도 자동 변경" : "Also apply monitor brightness/contrast", Location = new Point(110, 100), AutoSize = true };
                ...
                f.Controls.AddRange(new Control[] { lp, cp, le, te, br, am, ok, cancel }); f.AcceptButton = ok; f.CancelButton = cancel;
                if (f.ShowDialog(this) == DialogResult.OK && cp.SelectedItem != null && !string.IsNullOrWhiteSpace(te.Text))
                {
                    mappings.Add(new Mapping { Profile = cp.SelectedItem.ToString(), Exe = Path.GetFileName(te.Text.Trim()), Enabled = true, ApplyMonitor = am.Checked });
        ...
        private void Remove_Click(object sender, EventArgs e)
        {
            ...
            mappings.RemoveAt(list.SelectedIndex); changed = true; RefreshList();
        }
        ...
        private void Edit_Click(object sender, EventArgs e)
        {
            ...
            using (Form f = new Form())
            {
                ...
                Button br = new Button { Text = ko ? "찾기" : "Browse", Location = new Point(392, 59), Width = 68 };
                br.Click += delegate { using (OpenFileDialog d = new OpenFileDialog { Filter = "Executable (*.exe)|*.exe", Title = ko ? "게임 실행 파일 선택" : "Select game executable" }) { if (d.ShowDialog(f) == DialogResult.OK) te.Text = Path.GetFileName(d.FileName); } };
                CheckBox am = new CheckBox { Text = ko ? "모니터 밝기/대비도 자동 변경" : "Also apply monitor brightness/contrast", Location = new Point(110, 100), AutoSize = true, Checked = current.ApplyMonitor };
                Button ok = new Button { Text = ko ? "확인" : "OK", Location = new Point(300, 135), Width = 75, DialogResult = DialogResult.OK };
                Button cancel = new Button { Text = ko ? "취소" : "Cancel", Location = new Point(385, 135), Width = 75, DialogResult = DialogResult.Cancel };
                f.Controls.AddRange(new Control[] { lp, cp, le, te, br, am, ok, cancel });
                ...
                if (f.ShowDialog(this) == DialogResult.OK && cp.SelectedItem != null && !string.IsNullOrWhiteSpace(te.Text))
                {
                    ...
                    current.ApplyMonitor = am.Checked;
                    changed = true;
                    RefreshList();
        ...
        private void SaveMappings()
        {
            ...
            for (int i = 0; i < mappings.Count; i++)
            {
                ...
                ini.Write("ApplyMonitor", m.ApplyMonitor ? "1" : "0", section);
            }
