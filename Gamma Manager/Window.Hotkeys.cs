...
namespace Gamma_Manager
{
    ...
    public partial class Window : Form
    {
        ...
        private void RefreshGlobalHotkeys()
        {
            ...
            {
                string volHkText = iniFile.Read("VolumeDuckHotkey", "Settings");
                if (TryParseHotkey(volHkText, out Keys vKey, out GlobalHotkey.Modifiers vMod))
                {
                    int id = nextHotkeyId++;
                    GlobalHotkey hk = new GlobalHotkey(this.Handle, id, vKey, vMod);
                    hk.Pressed += delegate { ToggleVolumeDuck(); };
                    if (hk.Register())
                    {
                        globalHotkeys[id] = hk;
                        globalHotkeyPresets[id] = "__VOLUME_DUCK__";
                    }
                    else { hk.Dispose(); }
                }
            }
