...
namespace Gamma_Manager
{
    public partial class Window : Form
    {
        ...
        private int gameAutoSessionPid = 0;
        ...
        {
            return GetForegroundProcessName(out _);
        }

        private string GetForegroundProcessName(out int processId)
        {
            processId = 0;
        ...
        private bool IsProcessRunning(string exeName, int pid = 0)
        {
            if (string.IsNullOrEmpty(exeName)) return false;

            // 1단계: 기억된 PID로 O(1) 초고속 생존 및 프로세스명 확인
            if (pid > 0)
            {
                try
                {
                    using (Process p = Process.GetProcessById(pid))
                    {
                        if (!p.HasExited)
                        {
                            string targetName = Path.GetFileNameWithoutExtension(exeName);
                ...
                {
                    // 권한 부족 등의 경우 아래의 2단계 전체 검색으로 폴백
                }
            }

            // 2단계: PID가 없거나 확인 불가 시 전체 목록 검색
            try
            {
        ...
        public void EvaluateGameAutoState()
        {
            ...
            string foreground = GetForegroundProcessName(out int fgPid);
            Display.DisplayInfo fgDisplay = GetForegroundDisplay() ?? currDisplay;
            string fgMonitorKey = fgDisplay != null ? DisplayService.GetMonitorKey(fgDisplay) : string.Empty;

            // 게임 프로세스가 종료되었을 때 세션 정리 전 복원 보장
            if (!string.IsNullOrEmpty(gameAutoSessionProcess) && !IsProcessRunning(gameAutoSessionProcess, gameAutoSessionPid))
            {
                if (autoGameFocused && !string.IsNullOrEmpty(activeAutoMonitor))
                {
                    RestoreGameAutoPreviousMonitor(activeAutoMonitor);
