...
namespace Gamma_Manager
{
    ...
    {
        private static readonly SemaphoreSlim _displayChangeGate = new SemaphoreSlim(1, 1);
        ...
        private async void SystemEvents_DisplaySettingsChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Logger.Info("Display configuration changed. Scheduling re-enumeration...");
                ...
                // 비동기 작업이 겹쳐서 실행되지 않도록 락 진입
                await _displayChangeGate.WaitAsync(token);
                ...
                try
                {
                    ...
                    await Task.Run(() =>
                    {
                        ...
                        MonitorIdentity.AssignStableDisplayNames(freshDisplays);
                    }, token);
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            else if ((uint)m.Msg == Program.WM_DPM_RESTORE)
            {
