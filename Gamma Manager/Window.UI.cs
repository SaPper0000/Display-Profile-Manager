...
namespace Gamma_Manager
{
    public partial class Window : Form
    {
        ...
        public void UpdateOSDButtonState()
        {
            if (btnOsdSettings == null) return;

            bool isEnabled = IsOSDEnabled();
        ...
        public void UpdateShadowBoostButtonState()
        {
            if (btnShadowBoost == null) return;

            bool isEnabled = currDisplay != null && currDisplay.shadowBoost > 0;
            bool ko = LanguageManager.Korean;

            if (isEnabled)
            {
                btnShadowBoost.Text = ko ? "블랙 EQ\r\nON" : "Black EQ\r\nON";
                btnShadowBoost.ForeColor = Color.FromArgb(255, 75, 75); // 빨간색 ON
            }
            else
            {
                btnShadowBoost.Text = ko ? "블랙 EQ\r\nOFF" : "Black EQ\r\nOFF";
                btnShadowBoost.ForeColor = ThemeManager.IsDark
                    ? Color.FromArgb(180, 186, 198)
                    : Color.FromArgb(110, 115, 125); // 회색 OFF
            }
            ...
            {
                string tip = isEnabled
                    ? (ko ? $"블랙 이퀄라이저 [{currDisplay.shadowBoost}% ON]" : $"Black Equalizer [{currDisplay.shadowBoost}% ON]")
                    : (ko ? "블랙 이퀄라이저 [OFF]" : "Black Equalizer [OFF]");
                topMostToolTip.SetToolTip(btnShadowBoost, tip);
            }
        }
        ...
        {
            bool ko = LanguageManager.Korean;
            Text = "Display Profile Manager v1.5.4";
            notifyIcon.Text = "Display Profile Manager v1.5.4";
        ...
        private void SetupModernLayout()
        {
            ...
            // 게임 자동 우측에 동일한 크기(90x76)로 배치되는 블랙 EQ 버튼
            btnShadowBoost = new Button
            {
                Size = new Size(90, 76),
                Location = new Point(582, 50),
                FlatStyle = FlatStyle.Flat,
                TabStop = true,
                UseVisualStyleBackColor = false,
                Font = new Font("Segoe UI", 9.5f, FontStyle.Bold)
            };
            btnShadowBoost.Click += btnShadowBoost_Click;
            displayCard.Controls.Add(btnShadowBoost);
